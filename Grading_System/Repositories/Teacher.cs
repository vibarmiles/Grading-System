...
{
    internal class Teacher : BaseUserRepository, ITeacher
    {
        private string connectionString;
        private string specialization;

        public Teacher(string connectionString) : base(connectionString, "TeachersView", "[UserID]") 
        {
        ...
        public new void GetValues(string id)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT [Specialization] FROM Teachers WHERE [UserID] = @id", con))
                {
                    cmd.Parameters.Add("id", SqlDbType.BigInt);
                    cmd.Parameters["id"].Value = Int32.Parse(id);
